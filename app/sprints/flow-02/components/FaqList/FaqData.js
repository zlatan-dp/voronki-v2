export const FaqData = [
  {
    id: 1,
    question: "How much time does it take?",
    answer: "Just 7 minutes a day. Short, simple steps — nothing overwhelming.",
  },
  {
    id: 2,
    question: "What if I skip a day?",
    answer:
      "No problem at all. You can pick up right where you left off — no guilt, no pressure.",
  },
  {
    id: 3,
    question: "Is this a course?",
    answer:
      "No. There are no long lessons, no modules, no homework. You get small, guided steps in your messenger — something you can actually follow, even on tired days.",
  },
  {
    id: 4,
    question: "Is this a course?",
    answer:
      "Not in the traditional sense. ChatMND is a micro-learning experience: short lessons, practical steps, and zero overwhelm.",
  },
  {
    id: 5,
    question: "What exactly will I get?",
    answer: (
      <>
        <span>You’ll receive:</span>
        <br />
        <br />
        <span>• 5 × short up to 7-day sprints based on your quiz</span>
        <br />
        <span>• A simple step-by-step plan</span>
        <br />
        <span>• Daily guidance in your messenger</span>
        <br />
        <span>
          • Light, doable actions that help you feel more grounded and organized
        </span>
        <br />
        <br />
        <span>Just clear, manageable support — nothing heavy.</span>
      </>
    ),
  },
  {
    id: 6,
    question: "Will this actually help me stay consistent?",
    answer:
      "Yes — that’s the whole point. The steps are small, clear, and easy to follow, so consistency finally feels doable (not overwhelming).",
  },
  {
    id: 7,
    question: "Can I cancel anytime?",
    answer:
      "You’re always in control. Pause or cancel your plan at any moment — zero hassle, zero commitment.",
  },
  {
    id: 8,
    question: "Do I need any experience or previous knowledge?",
    answer:
      "No. Everything is beginner-friendly and designed for real life — not for experts.",
  },
  {
    id: 9,
    question: "What if I’m feeling really low right now?",
    answer:
      "You’re welcome here. The sprints aren’t medical treatment, but they offer gentle structure and small daily steps that can make things feel more manageable. No pressure — just support.",
  },
  {
    id: 10,
    question: "Is this a replacement for therapy?",
    answer:
      "No. If you’re going through something heavy, professional support can help.  ChatMND is here as a light daily companion, not medical care.",
  },
];
